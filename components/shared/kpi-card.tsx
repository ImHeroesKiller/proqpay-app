...
export function KpiCard({ item }: { item: KpiCardType; index?: number }) {
  ...
  const content = (
    <div className="group h-full">
      <div className="relative flex h-full min-h-[176px] flex-col overflow-hidden rounded-[22px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.06)] transition duration-200 group-hover:-translate-y-0.5 group-hover:border-blue-200 group-hover:shadow-[0_18px_38px_rgba(15,23,42,0.10)]">
        <div
          className={cn(
        ...
        <div className="relative flex items-start justify-between gap-3">
          <p className="max-w-[72%] text-xs font-bold uppercase leading-[1.35] tracking-[0.075em] text-slate-500">
            {item.label}
          </p>
          <span
            className={cn(
        ...
        <p className="relative mt-4 font-display text-[31px] font-extrabold leading-[1.05] tracking-[-0.04em] text-navy">
          {item.value}
        </p>
        {item.change ? (
          <p
            className={cn(
              "relative mt-auto flex items-center gap-1.5 pt-3 text-[13px] font-semibold leading-5",
          ...
          >
            ...
            <span className="line-clamp-2">{item.change}</span>
          </p>
        ) : null}
        <MetricSparkline label={item.label} />
      </div>
    </div>
  );
  ...
  return item.href ? (
    <Link
      href={item.href}
      className="block h-full rounded-[22px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
    >
      {content}
    </Link>
  ) : (
