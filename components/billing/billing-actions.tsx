...
import { createDraftInvoiceFromPayroll } from "@/lib/billing/actions";
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-2xl border bg-white p-4">
      ...
      <Button
        ...
        onClick={() =>
          start(async () => {
            const res = await createDraftInvoiceFromPayroll(periodId);
            setMsg(
              res.ok
                ? res.existing
                  ? "Draft invoice sudah ada untuk periode ini."
                  : `Draft ${res.invoiceNumber} dibuat.`
                : res.error,
            );
            if (res.ok && !res.existing) window.location.reload();
          })
        }
      >
        Buat draft invoice
